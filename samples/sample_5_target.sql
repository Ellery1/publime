...
FROM
  ...
  JOIN (
    SELECT
      tct.collection_task_no,
      sum (if (t.end_date>=current_date, 1, 0)) as notExpiredNum
    FROM
      core_ms.t_duebill_info t
      JOIN post_loan.t_collection_task tct ON tct.duebill_no = t.duebill_no
    WHERE
      t.op_flag != 'DELETE'
      AND t.duebill_status != 3
    GROUP BY
      tct.collection_task_no
  ) ep ON ep.collection_task_no = tct.collection_task_no
  and ep.notExpiredNum = 0
WHERE
  true
