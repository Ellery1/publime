...
FROM customer_stats cs WHERE order_count>0
)SELECT rc.*,u.username,u.email,u.registration_date
FROM ranked_customers rc JOIN users u ON rc.user_id=u.user_id
WHERE value_quartile IN(1,2);

/* 更新用户等级 */
UPDATE users SET user_level=CASE
WHEN user_id IN(SELECT user_id FROM top_customers WHERE value_quartile=1) THEN 'platinum'
WHEN user_id IN(SELECT user_id FROM top_customers WHERE value_quartile=2) THEN 'gold'
WHEN user_id IN(SELECT user_id FROM orders GROUP BY user_id HAVING COUNT(*)>=10) THEN 'silver'
ELSE 'bronze' END,
updated_at=NOW() -- 更新时间戳
WHERE is_active=1;

-- 删除过期数据
DELETE FROM order_logs WHERE created_at<DATE_SUB(NOW(),INTERVAL 90 DAY) AND log_type='debug';

/* 插入汇总数据 */
INSERT INTO daily_summary(summary_date,total_orders,total_revenue,avg_order_value,unique_customers)
SELECT DATE(order_date) as summary_date,COUNT(*) as total_orders,
SUM(total_amount) as total_revenue,AVG(total_amount) as avg_order_value,
COUNT(DISTINCT user_id) as unique_customers FROM orders
WHERE order_date>=CURDATE()-INTERVAL 1 DAY
GROUP BY DATE(order_date)
ON DUPLICATE KEY UPDATE total_orders=VALUES(total_orders),
total_revenue=VALUES(total_revenue),updated_at=NOW();
